...
from doc2json.utils.dblp_lookup_util import lookup_bibliography_entry_dblp_id
...
def get_bibliography_entry_dblp_id(title: str, other_ids: Dict[str, List]) -> Optional[str]:
    """
    Get DBLP ID for a bibliography entry from TEI XML.
    
    This function connects to MongoDB and looks up the bibliography entry
    using available identifiers (DOI, arXiv, title) in order of reliability.
    
    Args:
        title: Paper title extracted from TEI XML
        other_ids: Dictionary of identifiers (e.g., {"DOI": ["10.1234/..."], "arXiv": ["1234.5678"]})
    
    Returns:
        Normalized DBLP key (key_norm) if found, None otherwise
    """
    ...
    # Extract DOI and arXiv from other_ids
    if len(other_ids) != 0:
        for key, value in other_ids.items():
            if key == "DOI":
                doi = value[0] if value else None
            elif key == "arXiv":
                arxiv = value[0] if value else None
    
    # Use shared lookup function
    dblp_id = lookup_bibliography_entry_dblp_id(
        papers_collection,
        doi=doi,
        arxiv=arxiv,
        title=title
...
def parse_bib_entry(bib_entry: BeautifulSoup) -> Dict:
    ...
    dblp_id = get_bibliography_entry_dblp_id(title, other_ids)
    return {
